/* Juga i Aprèn! — app de repàs a l'estil Duolingo per a nens i nenes
   de 6 anys (1r de primària). Vanilla JS, sense dependències.
   Mascota: Robi, el robot ajudant. Sons generats amb Web Audio API.
   Tot el text es mostra en MAJÚSCULES via CSS (text-transform)
   perquè és més fàcil de llegir per a qui just comença a llegir. */

const STORAGE_KEY = "petits_state_v1";
const SOUND_KEY = "petits_sound_v1";
...
function resetProgress() {
  if (!confirm("Segur que vols esborrar tot el progrés?")) return;
  state = defaultState();
  saveState();
  goHome();
}
...
function renderHome() {
  ...
  wrap.appendChild(
    el("div", { class: "topbar" }, [
      el("div", { class: "brand" }, [document.createTextNode("🤖 Juga i Aprèn!")]),
      renderTopStats()
    ])
  );
  wrap.appendChild(
    el("div", { class: "hero" }, [
      el("h1", {}, "Hola! Vols jugar amb mi?"),
      el("p", {}, "1r de primària · 6 anys")
    ])
  );
  ...
  wrap.appendChild(
    el("div", { class: "footer-note" }, [
      document.createTextNode("Contingut pensat per a nens i nenes de 6 anys (1r de primària)."),
...
function renderBottomBar() {
  ...
  if (q.type === "match" && !session.answered) {
    ...
    return el("div", { class: "bottom-bar" }, [
      el("div", { class: "bottom-bar-inner" }, [
        el("div", { class: "match-hint" }, `🤖 Aparella-ho tot (${ms.matched.size}/${q.pairs.length})`)
      ])
    ]);
  }
  ...
  const inner = el("div", { class: "bottom-bar-inner" }, [
    el("div", { class: "feedback-text" }, [
      el("div", { class: `feedback-title ${ok ? "ok" : "bad"}` }, ok ? "🤖 Molt bé! 🎉" : "🤖 Quasi! 😕"),
...
function renderResults() {
  ...
  const title = stars === 3 ? "Perfecte!" : stars >= 1 ? "Molt bé!" : "Segueix jugant!";
  ...
  if (session.mistakes.length > 0) {
    const box = el("div", { class: "mistakes-box" }, [el("h3", {}, "🤖 Repassem junts")]);
